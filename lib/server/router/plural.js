...
module.exports = (db, name, opts) => {
  ...
  function embed(resource, e) {
    if (resource && e) {
      ...
      const resourceName = split.length > 1 ? _.camelCase(_.join(_.drop(split), '-')) : split[0];
      e.split(",").forEach(embedResource => {
        ...
        if (db.get(embedFullName).value) {
          ...
          resource[embedName] = db.get(embedFullName).filter(query).value();
        }
      });
    }
  }
  ...
  function join(resource, joinResource, foreignKey) {
    if (resource && joinResource && foreignKey) {
      ...
      if (joinName && joinFullName && fkValue) {
        resource[joinName] = db.get(joinFullName).getById(fkValue).value();
      }
    }
  }
  ...
  function list(req, res, next) {
    ...
    Object.keys(req.query).forEach(query => {
      ...
      for (let i in arr) {
        if (_.has(arr[i], query) || query === 'callback' || query === '_' || /_|\.lt$/.test(query) || /_|\.gt$/.test(query) || /_|\.le$/.test(query) || /_|\.ge$/.test(query) || /_|\.lte$/.test(query) || /_|\.gte$/.test(query) || /_|\.ne$/.test(query) || /_|\.like$/.test(query) || /_|\.null$/.test(query) || /_|\.empty$/.test(query) || /_|\.sort$/.test(query) || /_|\.join$/.test(query)) return;
      }
      ...
      if (key !== 'callback' && key !== '_' && !/_|\.(join|sort)$/.test(key)) {
        ...
        chain = chain.filter(element => {
          return arr.map(function (value) {
            ...
            const path = key.replace(/_|\.(lt|le|lte|gt|gte|ge|eq|ne|like|is_empty|is_null)$/, '');
